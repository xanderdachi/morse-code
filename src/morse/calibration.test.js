...
describe('calibrate', () => {
  it('measures the dot length of a perfect PARIS at any speed', () => {
    ...
    for (const wpm of [5, 10, 18, 25, 30]) {
      const result = calibrate(synthesizeKeying('PARIS', { wpm }))
  ...
  it('stays close with ±15% noise', () => {
    for (let seed = 1; seed <= 50; seed++) {
      const result = calibrate(synthesizeKeying('PARIS', { wpm: 16, jitter: 0.15, seed }))
  ...
  it('is not thrown by long hesitations between letters', () => {
    ...
    expect(calibrate(log).unitMs).toBeCloseTo(60, 0)
  })

  it('clamps to the estimator range', () => {
    expect(calibrate(synthesizeKeying('PARIS', { wpm: 33 }))).toMatchObject({ ok: true, unitMs: 40 })
    expect(calibrate(synthesizeKeying('PARIS', { wpm: 2.5 }))).toMatchObject({ ok: true, unitMs: 400 })
  })

  it('refuses anything that is not PARIS, reporting what it heard', () => {
    expect(calibrate(synthesizeKeying('PARTS', { wpm: 15 }))).toEqual({ ok: false, heard: 'PARTS' })
    expect(calibrate([])).toEqual({ ok: false, heard: '' })
  })

  it('refuses a PARIS tapped on the pad, which has no timing to measure', () => {
    expect(calibrate(synthesizeTapping('PARIS'))).toEqual({ ok: false, heard: 'PARIS' })
  })
})
